...
function updateDesignDoc(options, callback) {
  ...
  var design_doc_couchdb = {
    "views":{
      ...
      "ink_importer_by_oid_w_image_by_ctime": {
        //
        // key: <importer_id>, <0, 1, 2 depending upon whether import, original or variant>, <in trash>, <date>, <"" or image.name>, <'0' or original image id>
        //  note: Date is 7 fields -> key length is 12.
        //
        "map": "function (doc) { var key = []; if (doc.class_name === 'plm.InkImporter') { key.push(doc.oid, 0, 0, 0, 0, 0, 0, 0, 0, '', '0'); emit(key, doc.path); } if (doc.class_name === 'plm.Image') { if (doc.orig_id === '') { key.push(doc.batch_id, 1); } else { key.push(doc.batch_id, 2); } if (doc.in_trash) { key.push(1); } else { key.push(0); } var d = date_to_array(doc.created_at); key.push.apply(key, d); if (doc.orig_id === '') { key.push(doc.name, doc.oid); } else { key.push(doc.name, doc.orig_id); } emit(key, doc.name); } function date_to_array(aDate) { var out = [], d = new Date(aDate); out.push(d.getFullYear()); out.push(d.getMonth() + 1); out.push(d.getDate()); out.push(d.getHours()); out.push(d.getMinutes()); out.push(d.getSeconds()); out.push(d.getMilliseconds()); return out; }}",
        "reduce": "function(keys, values, rereduce) { var reduced = { num_images: 0, num_images_intrash: 0 }; if (rereduce) { for (var i = 0; i < values.length; i++) { var value = values[i]; reduced.num_images = reduced.num_images + value.num_images; reduced.num_images_intrash = reduced.num_images_intrash + value.num_images_intrash; } } else { var ni = 0; var nit = 0; for (var i = 0; i < keys.length; i++) { var key = keys[i][0]; if (key[1] === 1) { ni++; if (key[2] === 1) { nit++; } } } reduced.num_images = ni; reduced.num_images_intrash = nit; } return reduced; }"
      },
      "by_tag": {
